...
async def add_time_all_chat(session):
    ...
    for g_info in authed_group_list:
        await util.change_authed_time(g_info['gid'], days)
...
async def group_change_chat(session):
    ...
    group_time_old = util.check_group(old_gid)
    if group_time_old == 0:
    ...
    group_time_new = util.check_group(new_gid)
    msg = await util.process_group_msg(new_gid, expiration=group_time_new, title=f'旧群{old_gid}授权已清空, 新群授权状态：\n')
...
async def auth_status_chat(session):
    ...
    for sid in hoshino.get_self_ids():
        sgl = set(g['group_id'] for g in await session.bot.get_group_list(self_id=sid))
        frl = set(f['user_id'] for f in await session.bot.get_friend_list(self_id=sid))
        gp_num = len(sgl)
        fr_num = len(frl)
        msg += f'\nBot账号：{sid}\n所在群数：{gp_num}\n好友数：{fr_num}\n授权群数：{agp_num}\n'
...
async def no_number_check_chat(session):
    ...
    util.allow_list(group_id=gid, operator='add', nocheck='no_number_check')
...
async def no_auth_check_chat(session):
    ...
    if uid not in hoshino.config.SUPERUSERS:
        ...
        await session.finish('只有主人才能设置白名单')
        return
    util.allow_list(group_id=gid, operator='add', nocheck='no_auth_check')
...
async def no_check_chat(session):
    ...
    util.allow_list(group_id=gid, operator='add', nocheck='no_check')
...
async def remove_allow_list_chat(session):
    if not session.current_arg.strip():
    ...
    re_code = util.allow_list(group_id=gid, operator='remove')
    if re_code == 'not in':
...
async def get_allow_list_chat(session):
    if session.event.user_id not in hoshino.config.SUPERUSERS:
        util.log(f'{session.event.user_id}尝试查看白名单, 已拒绝')
        await session.finish('只有主人才能查看白名单')
        return

    allow_list = util.get_list(list_type='allow_list')
